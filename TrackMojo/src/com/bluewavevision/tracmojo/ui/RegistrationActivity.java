...
public class RegistrationActivity extends BaseActivity
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (isForSocialLogin) {
			Util.showAlertDialog(mContext, getString(R.string.app_name),
					getString(R.string.activity_registration_please_fill_required_detail));
		}
	}
	...
	private void initializeComponents() {
		...
		//
		// String first = "<font color='#6E6E6E'>I agree to the </font><font
		// color='#0177B5'>Terms of Use</font><font color='#6E6E6E'> and
		// </font><font color='#0177B5'>Privacy Policy</font>";
		//
		// tvPrivacyPolicy.setText(Html.fromHtml(first ));

		SpannableString SpanString = new SpannableString("I agree to the Terms of Use and Privacy Policy");
		...
		case R.id.activity_registration__btnSubmit:
			if (Util.isEditTextEmpty(etFirstName)) {
				...
				if (etCountryCode.getText().toString().trim().length() == 1  ) {
					...
					Util.showAlertDialog(mContext, getString(R.string.app_name),
							getString(R.string.activity_registration_check_privacy));
				} else {
					if (isForSocialLogin) {
	...
	private void registerUser() {
		...
		VolleyStringRequest registerRequest = new VolleyStringRequest(Method.POST, Webservices.REGISTER,
			...
			protected Map<String, String> getParams() throws com.android.volley.AuthFailureError {
				...
				params.put(PHONE_NUMBER, etCountryCode.getText().toString().trim().replace("+", "") + "-"
	...
	private void socialLogin() {
		...
		VolleyStringRequest socialLoginRequest = new VolleyStringRequest(Method.POST, Webservices.SOCIAL_LOGIN,
			...
			protected Map<String, String> getParams() throws com.android.volley.AuthFailureError {
				...
				// etCountryCode.getText().toString().trim()+etPhoneNumber.getText().toString().trim());
				params.put(DEVICE_TYPE, "a");
				params.put(DEVICE_ID, preferences.getString("DEVICE_ID", ""));
				params.put(LoginActivity.TIMEZONE, Util.getTimeZoneOffSet());
				params.put(LoginActivity.TIMEZONESTRING, Util.getCurrentTimeZone());
	...
	private final class ResponseListener implements DialogListener {
		...
		public void onComplete(Bundle values) {
			...
			if (Util.checkConnection(mContext)) {
				 ...
				 * if(!TextUtils.isEmpty(profile.getEmail())) tvEmail.setText(
				 * "Email : "+profile.getEmail()); else tvEmail.setText(
				 * "Email Not Found");
				 * 
				 * if(profile.getDob()!=null){ tvBirthday.setText("BirthDay : "
				 * +profile.getDob()); }
				 * if(!TextUtils.isEmpty(profile.getGender())) tvGender.setText(
				 * "Gender : "+profile.getGender());
				 * 
				 * if(!TextUtils.isEmpty(profile.getProfileImageURL())){
				 * tvProfileLink.setText("User Profile Link : "
				 * +profile.getProfileImageURL()); Log.e("url	",
				 * ":"+profile.getProfileImageURL().replace("_normal", ""));
				 * Map<String, String> contactInfo = profile.getContactInfo();
				 * if(contactInfo!=null){ Log.e("contact info size	",
				 * ":"+contactInfo.size()); } }
				 */
			}
	...
	public void showAlertDialog(Context context, String title, String message) {
		...
		// set dialog message
		alertDialogBuilder.setMessage(message).setCancelable(false).setPositiveButton(context.getString(R.string.ok),
				new DialogInterface.OnClickListener() {
					 ...
					 * button is clicked, just close // the dialog box and do
					 * nothing dialog.cancel(); } })
					 */;
